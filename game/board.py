...
class Board:
    ...
    def choosePropertyToMortgageDowngrade(self, player):
        ...
        for i in range(len(self.b)):
            ...
            if type(plot) == Property and not plot.isMortgaged and plot.owner == player:
                ownedStuff.append(
                    (i, plot.cost_base, self.b[i].isMonopoly, self.shareOfGroup(plot.group, player), plot.hasHouses))
        if len(ownedStuff) == 0:
            return False
    ...
    def listPropertyToBuild(self, player):
        ...
        for i in range(len(toBuildStuff) - 1, -1, -1):
            if toBuildStuff[i][3] > minInGroup[
                toBuildStuff[i][2]]:  # if it has more houses than minimum in that group, remove
    ...
    def improveProperty(self, player, availiableMoney):
        ...
        log.write(player.name + " builds house N" + str(self.b[propertyToImprove].hasHouses) + " on " + self.b[
            propertyToImprove].name, 3)
    ...
    def action(self, player, position, special=""):
        ...
        if type(self.b[position]) == Property:
            rent = self.calculateRent(position,
                                      special=special)  # calculate the rent one would have to pay (but not pay it yet)
        ...
        elif type(self.b[position]) == Chance or type(self.b[position]) == Community or type(
                self.b[position]) == PropertyTax:
